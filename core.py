    ...
    def process_comics(self, source_dir, dest_dir, api_key=None, is_move_operation=False, mode=1, dry_run=False):
        ...
        for count, file_path in enumerate(source_files, 1):
            ...
            if os.path.exists(target_file) and os.path.abspath(target_file) != os.path.abspath(file_path):
                ...
                # It's a duplicate if the exact byte-sizes match perfectly...
                # OR if the existing target matches the destination schema AND the source file is a known cached original.
        ...
        if cache_updated and not dry_run:
            ...
            if 'on_trash_prompt' in self.callbacks:
                ...
                if trash_it:
                    try:
                        ...
                        safe_log(f"Failed to trash files (ensure send2trash is installed): {e}")
                else:
